...
@section('content')
    ...
    <a class="btn btn-warning add m-2 rounded-4" href="{{ url('/staff/tambah') }}" role="button">
        ...
        <div class="card mb-2 w-100 rounded-0 {{ $count % 2 == 0 ? 'even' : 'odd' }}" style="max-height: 250px">
            <div class="row g-0">
                <div class="col-md-4" style="max-height: 250px; max-width: 400px;">
                    <img src="{{ asset('storage/' . $staff->foto) }}" class="img-fluid" alt="..."
                ...
                <div class="col-md-8 d-flex justify-content-evenly">
                    <div class="card-body align-self-center">
                        <h4 class="card-title mb-0 text-capitalize">
                            {{ $staff->staffName }}</h4>
                        <p class="card-text text-body-secondary">Jabatan: {{ $staff->role->roleName }}</p>
                    </div>
                    <div class="card-body align-self-center">
                        <a class="nav-links float-end text-decoration-none" href="{{ url('/staff/hapus/' . $staff->id) }}"
                            onclick="return confirm('Apa kamu yakin?')">Hapus</a>
                        <a class="nav-links float-end px-5 text-decoration-none"
                            href="{{ url('/staff/edit/' . $staff->id) }}">Edit</a>
                        <a class="nav-links float-end text-decoration-none"
                            href="{{ url('/staff/detail/' . $staff->id) }}">Detail</a>
                    </div>
                </div>
            </div>
        </div>
